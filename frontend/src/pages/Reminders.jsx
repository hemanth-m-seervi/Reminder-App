      ...
      <div className="bg-white rounded-xl shadow p-4 mb-6">
        ...
        <div className="flex gap-4 mb-4">
          <div className="w-full flex flex-col sm:flex-row flex-nowrap gap-4 items-start">
            ...
            <input name="deadline" value={form.deadline} onChange={handleChange} placeholder="Deadline" className="input min-w-0 w-32 ml-0 sm:ml-2" type="date" />
            <div className="flex gap-2 items-center">
              <button
                onClick={handleAdd}
                className="flex items-center gap-2 px-6 py-2 rounded-lg bg-gradient-to-r from-purple-500 to-purple-700 text-white font-semibold shadow-lg hover:from-purple-600 hover:to-purple-800 transition-all duration-200 focus:outline-none flex-shrink-0"
              ...
              <button
                className="px-2 py-1 rounded bg-red-500 text-white text-xs font-semibold hover:bg-red-600 transition"
        ...
        {nearest && !showMissed && (
          <div className={`rounded-xl p-4 mb-6 flex flex-col sm:flex-row justify-between items-center shadow-lg border-2 border-red-400 bg-red-50`}>
            <div>
            ...
            <button onClick={() => handleComplete(reminders.findIndex(r => r._id === nearest._id))} className="btn-secondary mt-2 sm:mt-0">Mark as Done</button>
          </div>
        )}
        {!showMissed && (
          <ul className="space-y-2">
            {rest.map((reminder, idx) => (
              <li key={reminder._id || idx} className={`rounded p-3 flex flex-col sm:flex-row justify-between items-center bg-purple-50`}>
                <span>
