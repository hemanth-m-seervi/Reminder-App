  ...
  return (
    <motion.div initial={{ opacity: 0, x: 40 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.5 }} className="max-w-2xl mx-auto">
      ...
      <div className="bg-white rounded-xl shadow p-6 mb-6">
        <div className="mb-8">
          <h3 className="text-xl font-bold text-purple-700 mb-2">Create or Select Exam Note</h3>
          <div className="flex gap-2 mb-4">
            <select value={selectedExam} onChange={e => setSelectedExam(e.target.value)} className="input rounded-lg border-2 border-purple-200 px-4 py-2">
              <option value="">Select Exam</option>
              {marksNotes.map((note, idx) => (
                <option key={idx} value={note.examName}>{note.examName}</option>
              ))}
            </select>
            <input
              value={newExamName}
          ...
          {selectedExam && (
            <div className="mb-4">
              <h4 className="text-lg font-semibold text-purple-700 mb-2">Add Subject to {selectedExam}</h4>
          ...
          {/* Search bar below create/select exam section */}
        ...
        <div className="mb-8">
          ...
          {[...marksNotes]
            ...
            .map((note, idx) => (
              <div key={idx} className={`bg-gradient-to-r from-blue-50 to-purple-50 rounded-xl p-4 mb-4 shadow cursor-pointer ${showDetails === note.examName ? 'border-2 border-purple-400' : ''}`}
                onClick={() => setShowDetails(showDetails === note.examName ? '' : note.examName)}>
                  ...
                  <table className="w-full text-left border border-purple-100 rounded-lg mb-2">
                    <thead>
                      <tr className="bg-purple-100 text-purple-700">
                        ...
                        {selectedExam === note.examName && <th className="py-2 px-3">Actions</th>}
                      </tr>
                    </thead>
                    <tbody>
